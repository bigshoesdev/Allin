<%@ Page Language="C#" MasterPageFile="게임관리.master" AutoEventWireup="true" CodeFile="머니선물내역.aspx.cs" Inherits="올인관리_머니선물내역" Title="코리아 게임 관리자페이지" %>
...
<script type="text/javascript">
    ...
    <asp:GridView ID="grdList" runat="server" AutoGenerateColumns="False" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="선물한 회원" SortExpression="NickName">
                <ItemTemplate>
                    <asp:HyperLink ID="lnkViewDetail" Text='<%# Eval("SentUserName") %>'  NavigateUrl='<%# "../User/회원이력정보.aspx?id=" + Eval("SentUID").ToString() + "&ReturnUrl=" + Request.Path %>' runat="server" CausesValidation="False"></asp:HyperLink>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Left" CssClass="GridCommandButton" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="선물받은 회원" SortExpression="NickName">
                <ItemTemplate>
                    <asp:HyperLink ID="lnkViewDetail1" Text='<%# Eval("ReceiveUserName") %>'  NavigateUrl='<%# "../User/회원이력정보.aspx?id=" + Eval("ReceiveUID").ToString() + "&ReturnUrl=" + Request.Path %>' runat="server" CausesValidation="False"></asp:HyperLink>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Left" CssClass="GridCommandButton" />
            </asp:TemplateField>
            <asp:BoundField DataField="sentChips" HeaderText="금액" SortExpression="GameMoney" DataFormatString="{0:N0}">
