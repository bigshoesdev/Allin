<%@ Page Language="C#" MasterPageFile="../EnterpriseExchange/업체입출금관리.master" EnableEventValidation="false" AutoEventWireup="true" CodeFile="로링식업체별정산.aspx.cs" Inherits="Bonsa_EnterpriseExchange_로링식업체별정산" %>
...
&nbsp;<asp:Button ID="btn7" runat="server" CommandArgument="6" class="btn btn-success" EnableViewState=false
    ...
    <asp:GridView ID="grdList" runat="server" AutoGenerateColumns="False"
        ...
        <Columns>
            <asp:TemplateField HeaderText="업체명" SortExpression="id">
                <ItemTemplate>
                    <asp:HyperLink ID="EntName" runat="server" ToolTip="하층업체보기" Text='<%# Eval("name").ToString() %>' NavigateUrl='<%# "./로링식업체별정산.aspx?ent_id=" + Eval("id").ToString() %>'></asp:HyperLink>
                </ItemTemplate>
            </asp:TemplateField>
           
            <asp:BoundField DataField="a_betMoneySum" HeaderText="올인베팅금액" DataFormatString="{0:N0}">
            ...
            <asp:BoundField DataField="all_betMoneySum" HeaderText="총베팅금액" DataFormatString="{0:N0}">
              ...
              <asp:BoundField DataField="service_percent" HeaderText="수수료율" DataFormatString="{0:N1}%">
                <ItemStyle HorizontalAlign="right" />
                <FooterStyle HorizontalAlign="right" />
            </asp:BoundField>
