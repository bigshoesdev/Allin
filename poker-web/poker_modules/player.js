...
var Player = function (socket, name, chips, mID, mPhotoID) {
    this.public = {
        ...
        bet: 0,
        betcopy: 0,
        // The win number of the player in 
        winNumber: 0,
        // The lost number of the player
        lostNumber: 0,
        // The avarta number of the player 
        avartaNo: mPhotoID,
        // The player state
        state: 0,
        startChip: 0,
    ...
    // The hand that the player has in the current poker round and its rating
    this.evaluatedHand = {};
    this.evaluatedHand_last = {};
    this.id = mID;
    this.joinRoomChip = 0;
    this.leaveRoomState = false;
}
...
Player.prototype.leaveTable = async function () {
    if (this.sittingOnTable !== false) {
        ...
        await userService.updateChipsByAdd(this.id, 1 * this.public.chipsInPlay + 1 * this.joinRoomChip);
...
Player.prototype.sitOnTable = function (tableId, seat, chips) {
    ...
    this.public.chipsInPlay = this.joinRoomChip;
    this.public.startChip = this.public.chipsInPlay;
    this.joinRoomChip = 0;
 ...
 * @param  number   chips   
 */
Player.prototype.sitChangeOnTable = function (tableId, seat, chips) {
    // Remove the chips that player will have on the table, from the player object
...
Player.prototype.sitOut = function () {
    if (this.sittingOnTable !== false) {
        ...
        this.public.evaluatedHand = '';
        this.public.evaluatedHand_last = '';
        this.public.cards = [];
...
Player.prototype.bet = function (amount) {
    ...
    this.public.betcopy += amount;
    this.public.last_bet = amount;
}
...
Player.prototype.prepareForNewRound = function () {
    ...
    this.public.hasCards = false;
    this.public.bet = 0;
    this.public.betcopy = 0;
    this.public.last_bet = 0;
...
/**
 * Returns the player's rank and its rating only when 2 cards 
 * @param array cards (the cards that are in player's hand)
 * @return object this (for chaining)
*/

Player.prototype.evaluateHand_twoCards = function (cards) {

    var cardNamess = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'];
    var cardNames = { 'A': 'A', 'K': 'K', 'Q': 'Q', 'J': 'J', 'T': '10', '9': '9', '8': '8', '7': '7', '6': '6', '5': '5', '4': '4', '3': '3', '2': '2' }
    var evaluatedHand = {
        'rank': '',
        'name': '',
        'rating': 0,
    ...
    var rateHand = function (cards) {
        if (cardNamess.indexOf(cards[1][0]) > cardNamess.indexOf(cards[0][0])) {
            return cardNamess.indexOf(cards[1][0]) * 14 + cardNamess.indexOf(cards[0][0]);
        } else {
            return cardNamess.indexOf(cards[0][0]) * 14 + cardNamess.indexOf(cards[1][0]);
        }
    }

    if (cardNamess.indexOf(cards[0][0]) == cardNamess.indexOf(cards[1][0])) {
        evaluatedHand.rank = 'pair';
        //        evaluatedHand.name = 'a pair of ' + getCardName( cards[0][0], true );
        evaluatedHand.name = getCardName(cards[0][0]) + ' 원패어';

        evaluatedHand.rating = rateHand(cards) + 1000000;
        evaluatedHand.cards = cards;

    } else {
        evaluatedHand.rank = 'high card';
        var highCard = cardNamess.indexOf(cards[0][0]) > cardNamess.indexOf(cards[1][0]) ? cards[0][0] : cards[1][0];
        //        evaluatedHand.name = getCardName( highCard ) + ' high';
        evaluatedHand.name = getCardName(highCard) + ' 탑';
        evaluatedHand.rating = rateHand(cards)
        evaluatedHand.cards = cards;
    }
 ...
 * @param number boardCardNumber (board cards number)
 * @return object this (for chaining)
 */
Player.prototype.evaluateHand_last = function (board, boardCardNumber) {
    ...
    global.log("============evaluateHand_Last================" + this.public.name);
    global.log(cards);
    ...
    for (var i = 1; i < 2 + boardCardNumber; i++) {
        //    for( var i=1 ; i<7 ; i++ ) {
    ...
    switch (evaluatedHand.rank) {
        ...
        case 'two pair':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ',' + getCardName(evaluatedHand.cards[2][0]) + ' 투페어';
        ...
        case 'three of a kind':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ' 트리플';
            evaluatedHand.rating = rateHand(evaluatedHand.cards) + 3000000;
            break;
        case 'straight':
            //            evaluatedHand.name = 'a straight to ' + getCardName( straight[0][0] );
            evaluatedHand.name = getCardName(straight[0][0]) + ' 스트레이트';

            if (evaluatedHand.name == 'A 스트레이트')
                evaluatedHand.name = '마운틴';
        ...
        case 'flush':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ' 플러쉬';
        ...
        case 'full house':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ',' + getCardName(evaluatedHand.cards[3][0]) + ' 풀하우스';
        ...
        case 'four of a kind':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ' 포카드';
        ...
        case 'straight flush':
            ...
            evaluatedHand.name = getCardName(evaluatedHand.cards[0][0]) + ' 스트레이트 플러쉬';
            evaluatedHand.rating = rateHand(evaluatedHand.cards) + 8000000;
            break;
        case 'royal flush':
            //            evaluatedHand.name = 'a royal flush';
            evaluatedHand.name = '로얄 플러쉬';
            evaluatedHand.rating = rateHand(evaluatedHand.cards) + 8000000;
            break;
    }
    this.evaluatedHand_last = evaluatedHand;
}
 ...
 * @return object this (for chaining)
 */
Player.prototype.evaluateHand = function (board, boardCardNumber) {
    ...
    global.log("============evaluateHand================" + this.public.name);
    global.log(cards);
    // Getting the suit of the first card
    flushes[cards[0][1]].push(cards[0]);
    // Pushing the first card in the array of the straight
