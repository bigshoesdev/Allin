...
Pot.prototype.addTableBets = function (players) {
  global.log('---Called \"addTableBets\" func in pot.js------');
  global.log(this.pots);

  players.forEach(player => {
    if (player != null) {
      global.log('player ' + player.public.name + ' ' + player.public.chipsInPlay);
    }
  });
  ...
  if (allBetsAreEqual) {
    for (var i in players) {
      if (players[i] && players[i].public.bet) {
        ...
        players[i].public.betcopy = players[i].public.betcopy;
...
Pot.prototype.destributeToWinners = function (players, firstPlayerToAct) {
  global.log('---Called \"destributeToWinners\" func in pot.js------');
  global.log(this.pots);

  players.forEach(player => {
    if (player != null) {
      global.log('player ' + player.public.name + ' ' + player.public.chipsInPlay + ' ' + player.public.inHand + ' ' + player.evaluatedHand.rating);
    }
  });

  global.log("FirstPlayerToAct " + firstPlayerToAct);

  var potsCount = this.pots.length;
  var messages = [];
  var winners = [];
  var playerEarnings = [0, 0, 0, 0, 0, 0, 0, 0, 0];
  var roundFee = 0;
  var earning = 0;
  ...
  for (var i = potsCount - 1; i >= 0; i--) {
    ...
    global.log("Winners " + winners);

    let managerFee = 0;

    if (this.pots[i].contributors.length > 1) {
      managerFee = global.managerFee;
    }

    if (winners.length === 1) {
      // get winner earning and admin fees
      earning = parseInt(this.pots[i].amount * (100 - managerFee) / 100);
      players[winners[0]].public.chipsInPlay += earning;
      playerEarnings[players[winners[0]].seat] = this.pots[i].amount;

      roundFee = parseInt(this.pots[i].amount) - earning;

      var htmlHand = '[' + players[winners[0]].evaluatedHand.cards.join(', ') + ']';
      htmlHand = htmlHand.replace(/s/g, '&#9824;').replace(/c/g, '&#9827;').replace(/h/g, '&#9829;').replace(/d/g, '&#9830;');
      messages.push(players[winners[0]].public.name + ' wins the pot (' + earning + ') with ' + players[winners[0]].evaluatedHand.name + ' ' + htmlHand);

      players[winners[0]].public.winNumber = 1
      players[winners[0]].public.lostNumber = 0;
    } else {
      ...
      for (var j in winners) {
        ...
        // get winner earning and admin fees
        earning = parseInt(playersWinnings * (100 - managerFee) / 100);
        players[winners[j]].public.chipsInPlay += earning
        //        winnerEarings.push(earning);
        playerEarnings[players[winners[j]].seat] = playersWinnings;
        roundFee += parseInt(playersWinnings) - earning;

        var htmlHand = '[' + players[winners[j]].evaluatedHand.cards.join(', ') + ']';
        htmlHand = htmlHand.replace(/s/g, '&#9824;').replace(/c/g, '&#9827;').replace(/h/g, '&#9829;').replace(/d/g, '&#9830;');
        messages.push(players[winners[j]].public.name + ' ties the pot (' + earning + ') with ' + players[winners[j]].evaluatedHand.name + ' ' + htmlHand);

        players[winners[j]].public.winNumber = 1
        // Decrease winner's lostNumber because of start increasement.
        players[winners[j]].public.lostNumber = 0;
      }
    }
  }
  ...
  for (var j = 0; j < playersCount; j++) {
    if (players[j]) {
      global.log(players[j].public.name + ' winNum = ' + players[j].public.winNumber + ', lostNum = ' + players[j].public.lostNumber);
      userService.updateWinLostNumber(players[j].public.name, players[j].public.winNumber, players[j].public.lostNumber);
    }
  }
  ...
  var totalAmount = 0;
  for (var j = 0; j < 9; j++) {
    totalAmount += playerEarnings[j];
  }

  return { messages: messages, winners: winners, playerEarnings: playerEarnings, roundFee: roundFee, totalAmount: totalAmount };
}
 ...
 * @param object  winner
 */
Pot.prototype.giveToWinner = function (winner, winnerSeat) {
  global.log('---Called \"giveToWinner\" func in pot.js------');

  console.log(winner.public);
  
  if (!winner)
    return;

  var messages = [];
  var winners = [winnerSeat];
  var playerEarnings = [0, 0, 0, 0, 0, 0, 0, 0, 0];
  ...
  var last_bet = winner.public.last_bet;

  for (var i = potsCount - 1; i >= 0; i--) {
    totalAmount += this.pots[i].amount;
  }

  winner.public.chipsInPlay += parseInt(((totalAmount - last_bet) * (100 - managerFee) / 100) + last_bet);
  this.reset();
  global.log('^^^^^^winner : ' + winner.public.name);

  var roundFee = parseInt(totalAmount * managerFee / 100.0);
  playerEarnings[winnerSeat] = totalAmount;
  return { messages: [], winners: winners, playerEarnings: playerEarnings, roundFee: roundFee, totalAmount: totalAmount };
